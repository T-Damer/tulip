...
export default function ImageScroll({ items }: { items: Media[] }) {
  return (
    <div
      className="flex w-full snap-x snap-mandatory flex-row overflow-x-scroll border-b-2 sm:mt-0 dark:border-b-white"
      onWheel={(ev) => {
    ...
    >
      ...
      {items.map((item) => (
        <div className="shrink-0 snap-center border-l-2 dark:border-l-white">
          <Zoom>
            <img
      ...
      <div className="bg-stripes dark:border-white-pale h-full w-12.5 shrink-0 snap-center border-l-2" />
    </div>
  )
}
